...
> **注意(ファイル権限)**: `bme280.json` はアトミックwrite(一時ファイル→リネーム)で書かれるため、
> 所有者のみ読み書き可能な権限(`0600`)になります。本サービスと
> `raspi-esp32-status-panel` ハブを**別々のUnixユーザーで動かす場合**、ハブ側が
> `bme280.json` を読めずに詰まる可能性があります。同じユーザー(例: `pi`)で
> 両方動かすか、別ユーザーにする場合はパーミッション/グループ共有を別途検討してください。
